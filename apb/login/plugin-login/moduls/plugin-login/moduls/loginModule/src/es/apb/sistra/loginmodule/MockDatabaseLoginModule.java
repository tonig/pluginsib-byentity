...
public class MockDatabaseLoginModule extends DatabaseServerLoginModule {
	...
	protected Principal createIdentity(String username) throws Exception {
		...
		if (username != null && username.equals( (String) options.get("unauthenticatedIdentity"))){
			log.debug ("Creating ApbPrincipal para "+username);
			return new ApbPrincipal(username,"","",'A');
		}
		...
		{			
			log.debug ("Creating ApbPrincipal para "+username);
			return new ApbPrincipal(username,ui.fullName,ui.nif,'U');
		}
	 ...
	 protected Group[] getRoleSets() throws LoginException
	   {
	      ...
	      {
	      	if (log.isTraceEnabled()) log.trace("Principal "+principal+" not a ApbPrincipal");
	      	return new Group[0];
	      }
