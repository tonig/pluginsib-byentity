...
public class DatabaseLoginModuleStm extends DatabaseServerLoginModule {
	...
	protected Principal createIdentity(String username) throws Exception {
		...
		if (username != null && username.equals( (String) options.get("unauthenticatedIdentity"))){
			log.debug ("Creando Principal para "+username);
			return new StmPrincipal(username,"","",'A');
		}
		...
		{			
			log.debug ("Creando Principal para "+username);
			return new StmPrincipal(username,ui.fullName,ui.nif,'U');
		}
	 ...
	 protected Group[] getRoleSets() throws LoginException
	   {
	      ...
	      if ( ! (principal instanceof StmPrincipal) )
	      {
	      	if (log.isTraceEnabled()) log.trace("Principal "+principal+" not a STMPrincipal");
	      	return new Group[0];
	      }
